...
class LocalRuntimeTests(unittest.TestCase):
    def test_run_command(self):
        self.assertEqual(['Test Success!'],
                         dev.Runtime.run_command(['/bin/echo', 'Test Success!'], capture_output=True))
    ...
    def test_local_runtime_setup(self):
        ...
        self.assertFalse(dev.Runtime.setup(location=os.path.join(test_data_dir, 'local_runtime_example_bad')))
...
class DockerRuntimeTests(unittest.TestCase):
    ...
    def test_docker_setup(self):
        ...
        self.assertRaises(subprocess.CalledProcessError,
                          dev.DockerRuntime.setup, location=os.path.join(test_data_dir, 'test_root'), name=self.image_name)

    def test_docker_run(self):

        self.assertIn('NAME="Alpine Linux"',
                      dev.DockerRuntime.run_command(self.image_name, ['cat', '/etc/os-release'], capture_output=True))

        self.assertRaises(subprocess.CalledProcessError,
                          dev.DockerRuntime.run_command, name='bad', command=['cat', '/etc/os-release'], capture_output=True)
